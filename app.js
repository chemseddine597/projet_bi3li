// =============================================
//  Bi3li — App.js (Complete Enhanced)
...
function renderAuthPage(mode) {
  ...
  if (mode === 'login') {
    ...
    document.getElementById('registerForm').addEventListener('submit', e => {
      ...
      const result = DB.registerUser({ name, email, password: pass, role, location });
      if (result.error) {
        showFieldError(document.getElementById('regEmail'), result.error);
        return;
      }
      DB.setSession(result.user.id);
...
function renderStoreDashboard() {
  ...
  const main = document.getElementById('mainContent');
  const maxRev = Math.max(...s.revenueChart);
    ...
    <div class="anim-in">
      <div class="page-header">
        <div class="page-header-left">
          ...
          <h1>Bi3li</h1>
          <p>Curated second-hand marketplace · Founded 2023</p>
        </div>
      ...
      <div class="stats-grid">
        <div class="stat-card" style="--accent-color:#1a6b4a">
          ...
          <div class="sc-trend trend-up">↑ +18% vs last month</div>
        </div>
...
// =============================================
function renderProductsManager(sellerId = null) {
  const main = document.getElementById('mainContent');
  let filterCat = 'all', filterStatus = 'all', filterDate = '', sortCol = 'postedDate', sortDir = -1, searchQ = '';
  pagination.products.page = 1;

  function getFiltered() {
    let list = sellerId ? DB.getProductsBySeller(sellerId) : DB.getProducts();
    if (filterCat !== 'all') list = list.filter(p => p.category === filterCat);
    if (filterStatus !== 'all') list = list.filter(p => p.status === filterStatus);
    if (filterDate) list = list.filter(p => p.postedDate >= filterDate);
    if (searchQ) {
  ...
  function renderTable() {
    ...
    tbody.innerHTML = list.length ? list.map(p => `
      <tr>
        ...
        <td style="font-size:13px;color:var(--ink-muted);">${fmtDate(p.postedDate)}</td>
    ...
    <div class="anim-in">
      ...
      <div class="card" style="padding:0;overflow:hidden;">
        <div class="table-wrap">
          <table>
            <thead><tr>
              <th class="sort-th" data-col="title" style="cursor:pointer;">Product <span class="sort-arrow"></span></th>
              <th class="sort-th" data-col="category" style="cursor:pointer;">Category <span class="sort-arrow"></span></th>
              <th class="sort-th" data-col="price" style="cursor:pointer;">Price <span class="sort-arrow"></span></th>
              <th>Location</th>
              <th class="sort-th" data-col="postedDate" style="cursor:pointer;">Date <span class="sort-arrow"></span></th>
              <th class="sort-th" data-col="status" style="cursor:pointer;">Status <span class="sort-arrow"></span></th>
              <th>Actions</th>
            </tr></thead>
  ...
  renderTable();
  document.getElementById('prodCount').textContent = `${DB.getProducts().length} products`;
...
function renderPagination(containerId, total, page, perPage, onPage) {
  ...
  if (!el) return;
  const pages = Math.max(1, Math.ceil(total / perPage));
  if (pages <= 1) { el.innerHTML = `<span style="font-size:12px;color:var(--ink-muted);">${total} result${total!==1?'s':''}</span>`; return; }
  const btns = [];
  for (let i = 1; i <= pages; i++) {
    btns.push(`<button onclick="${onPage.name||''}(${i})" style="width:32px;height:32px;border-radius:6px;border:1.5px solid ${i===page?'var(--green)':'var(--border)'};background:${i===page?'var(--green)':'white'};color:${i===page?'white':'var(--ink-soft)'};font-size:13px;cursor:pointer;font-weight:600;">${i}</button>`);
  }
  // Use inline click handlers via a global fn
  el._onPage = onPage;
...
function saveNewProduct() {
  ...
  const sid      = sellerEl ? sellerEl.value : currentUser.id;
  const seller   = DB.getUserById(sid);
  ...
  DB.addProduct({
    ...
    brand: document.getElementById('fp-brand').value.trim() || 'Unknown',
...
function openProductDetailModal(id) {
  const p = DB.getProductById(id);
  if (!p) return;
  const seller = DB.getUserById(p.sellerId);
  ...
  openModal(null, `
    <div style="display:grid;grid-template-columns:1fr 1fr;">
      ...
      <div style="padding:32px 28px;">
        ...
        <div style="font-size:12.5px;color:var(--ink-muted);margin-bottom:4px;">${escHtml(p.condition)}${p.size!=='—'?' · Size '+escHtml(p.size):''} · ${escHtml(p.brand)}</div>
        <div style="font-size:12px;color:var(--ink-muted);margin-bottom:16px;">📍 ${escHtml(p.location||'—')} · 📅 ${fmtDate(p.postedDate)}</div>
...
function handleBuy(id) {
  ...
  if (currentUser.role === 'buyer') {
    if (currentUser.balance < p.price) { showToast('Insufficient balance!', 'error'); return; }
    DB.updateUser(currentUser.id, { balance: currentUser.balance - p.price });
    currentUser.balance -= p.price;
  }
  DB.updateProduct(id, { status: 'sold', likes: (p.likes||0)+1 });
  closeModal();
  showToast(`🎉 "${p.title}" purchased for $${p.price}!`, 'success');
}
  ...
  function renderTable() {
    ...
    tbody.innerHTML = list.length ? list.map(u => `
      <tr>
        ...
        <td style="font-size:13px;color:var(--ink-muted);">${fmtDate(u.createdAt)}</td>
        <td><span class="pill pill-green">${escHtml(u.badge)}</span></td>
        <td>
          <div style="display:flex;gap:6px;">
            ${u.id !== currentUser.id ? `<button class="btn btn-sm btn-red" onclick="adminDeleteUser('${u.id}')">Remove</button>` : '<span style="font-size:12px;color:var(--ink-muted);">You</span>'}
          </div>
        </td>
      </tr>
    ...
    <div class="anim-in">
      <div class="page-header">
        <div class="page-header-left">
          ...
          <h1>All Users</h1>
          <p>${DB.getUsers().length} registered members</p>
        </div>
      </div>
...
// =============================================
//  VIEW: SELLER PROFILE (with edit + password + delete account)
// =============================================
function renderSellerProfile(isSelf) {
  const main = document.getElementById('mainContent');
  const user = currentUser;
  const myProducts = DB.getProductsBySeller(user.id);
    ...
    <div class="anim-in">
      <div class="profile-hero">
        ...
        <div class="ph-info">
          ...
          <div class="ph-meta">📍 ${escHtml(user.location||'—')} · Member since ${fmtDate(user.createdAt)}</div>
          ...
          <div class="ph-stats">
            ...
            <div class="ph-stat"><div class="ph-stat-val">$${user.balance.toLocaleString()}</div><div class="ph-stat-lbl">Balance</div></div>
          </div>
      ...
      <div style="margin-bottom:16px;">
        ...
        <div class="products-grid">
          ${myProducts.length ? myProducts.map(p => productCardHTML(p, true)).join('') : `<div class="empty-state" style="grid-column:1/-1"><div class="es-icon">📦</div><h3>No listings yet</h3><p>Add your first item!</p></div>`}
        </div>
      </div>
    </div>
  `;
...
window.saveProfile = function() {
  ...
  DB.updateUser(currentUser.id, {
    name, location: document.getElementById('ep-loc').value.trim(),
    bio: document.getElementById('ep-bio').value.trim(),
  });
  currentUser.name     = name;
  currentUser.location = document.getElementById('ep-loc').value.trim();
  currentUser.bio      = document.getElementById('ep-bio').value.trim();
  document.getElementById('sidebarUser').querySelector('.su-name').textContent = name;
  closeModal();
...
window.changePassword = function() {
  ...
  if (curr !== currentUser.password) {
    showFieldError(document.getElementById('pwdCurrent'), 'Current password is incorrect.');
    return;
  }
  if (newPwd.length < 6) {
  ...
  DB.updateUser(currentUser.id, { password: newPwd });
...
window.confirmDeleteAccount = function() {
  ...
  if (pwd !== currentUser.password) { errEl.textContent = 'Incorrect password.'; return; }
    ...
    <div class="anim-in" style="max-width:720px;">
      ...
      <div class="card">
        <form class="form-grid" id="addItemForm" novalidate>
          ...
          <div class="form-row full"><label>Description *</label><textarea id="ap-desc" rows="4" placeholder="Describe your item honestly — condition, history, any flaws…" required></textarea></div>
        </form>
...
function submitNewListing() {
  ...
  DB.addProduct({
    ...
    brand: document.getElementById('ap-brand').value.trim() || 'Unknown',
...
function renderBuyerWishlist() {
  const main = document.getElementById('mainContent');
  const wished = DB.getProducts().filter(p => likedIds.has(p.id));
  main.innerHTML = `
    <div class="anim-in">
      <div class="page-header"><div class="page-header-left">
      ...
      <div class="products-grid">
        ${wished.length ? wished.map(p => productCardHTML(p)).join('') : `<div class="empty-state" style="grid-column:1/-1"><div class="es-icon">❤️</div><h3>Your wishlist is empty</h3><p>Click ❤️ on any item to save it here.</p></div>`}
      </div>
    </div>
  `;
  main.querySelectorAll('.view-detail-btn').forEach(btn => {
...
function renderStorefront() {
  ...
  function filteredProds() {
    ...
    if (sortBy === 'newest')      list = [...list].sort((a,b) => b.postedDate.localeCompare(a.postedDate));
    else if (sortBy === 'oldest') list = [...list].sort((a,b) => a.postedDate.localeCompare(b.postedDate));
  ...
  main.innerHTML = `
    <div class="anim-in">
      <div class="storefront-hero">
        <div class="sfh-eyebrow">✦ ${DB.getProducts().filter(p=>p.status==='available').length} items available today</div>
...
function productCardHTML(p, showEdit = false) {
  ...
  const badgeHTML = p.badge ? `<div class="pill pill-${p.badge==='new'?'green':p.badge==='hot'?'amber':'blue'}" style="position:absolute;top:10px;left:10px;font-size:10px;">${escHtml(p.badgeLabel)}</div>` : '';
  const statusHTML = p.status === 'sold' ? `<div class="pill pill-gray" style="position:absolute;top:10px;right:10px;font-size:10px;">Sold</div>` : '';
  ...
  return `
    <div class="product-card">
      <div class="pc-img">
        ...
        ${badgeHTML}
        ${statusHTML}
        <button class="like-product-btn" data-id="${p.id}" style="position:absolute;bottom:10px;right:10px;width:32px;height:32px;border-radius:50%;background:${isLiked?'#fff0f0':'white'};border:none;cursor:pointer;font-size:15px;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.1);transition:all 0.2s;">${isLiked?'❤️':'🤍'}</button>
      </div>
      <div class="pc-body">
